...
* __Input] XML-Wurzelelement des Datensatzbeschreibungsdokuments. Benötigt mindestens ein Rating-Element.
* __Location (String)__ In Typ File: Speicherort als absoluter Pfad in Dateisystem.
* __MetaFile] Erbt von File: Beschreibt eine CSV-Datei die Metaattribute speichert. Benötigt zu den File-Parametern die Angabe IdColumnNumber.
...
* __NominalMultivaluedAttribute] Erbt von NominalAttribute: Beschreibt ein Metaattribut das nominal verarbeitet werden soll und dessen Attributwerte mehrer Werte haben kann. Beispielsweise das Genre eines Films: Toy Strory, Children, Animation
...
* __PreProcessingRegex (String, optional)__ In Typ Attribute: Alle eingelesenen Attributwerte werden mit \texttt{attributeValueString.replaceAll(regex, emptyString)} gefiltert
...
\begin{quote}
\texttt{java -Xss100m -Xmx2048M -jar Agglorecommender.jar -p </path/to \newline /your/dataset-properties-file.xml>}
\end{quote}
Den Parameter  \texttt{-p} oder \texttt{-datasetProperties} mit dem darauf folgendem Pfad zu einer gültigen Datensatzdefinitionsdatei muss bei jedem Start der Applikation mitgegeben werden. Für Testzwecke kann anstelle einer Datensatzdefinitionsdatei auch das Schlüsselwort \texttt{random} angegeben werden. Damit wird ein Datensatz zufällig generiert. Es besteht auch die Möglichkeit die Grösse des zufällig generierten Datensatzes zu bestimmen:
\begin{quote}
\texttt{java -Xss100m -Xmx2048M -jar Agglorecommender.jar -p random \newline <number-of-users> <number-of-content-items> \newline <approx. percentage of null entries in User-Content-Rating-Matrix>}
\end{quote}
Zusätzlich kann die Applikation mit folgenden optionalen Kommandozeilenargumenten gestartet werden:
\begin{description}
\item[\texttt{-nodeUpdater}, \texttt{-nU}, \texttt{-updater}] Erwartet den Klassennamen einer Klasse des Typs INodeUpdater. Standardwert: \texttt{SaveAttributeNodeUpdater} (siehe Abbildung~\ref{fig:TreeUpdate} für Erklärung der Funktionsweise). Erlaubte Werte: \texttt{ExtendedNodeUpdater}: Ergibt das gleiche Klassifizierungsergebnis wie \texttt{SaveAttributeNodeUpdater}, \texttt{NullUpdater}: Es werden keine Klassifizierungsinformationen zwischen den zwei Klassenhiererchien ausgetauscht, \texttt{SimpleNodeUpdater}: Neue Klassen werden als Attribute in die Klassen der nicht erweiterten Klassenhierarchie eingebaut. Die Kinder der neuen Klasse werden aber nicht als Attribute der Klassen der nicht erweiterten Klassenhiererchie entfernt.
\item[\texttt{-noGUI}] Clustering läuft ohne graphische Darstellung der Klassenbäume und des Klassifizierungsfortschritts ab.
\item[\texttt{-client}] Erwartet den Klassennamen einer Klasse des Typs IClient. Standardwert: \texttt{DefaultClient}. Erlaubte Werte: \texttt{WebClient}: Öffnet eine Empfehlungswebsite welche über http://localhost:8081 aufgerufen werden kann. 
\item[\texttt{-resume}, \texttt{-r}] Erwartet den Dateipfad zum Speicherort eines serialisierten TreeBuilder-Objekts. Lädt den TreeBuilder von der Festplatte und setzt bei Bedarf einen unterbrochenen Klassifizierungsvorgang fort.
